...
mha_fwd_kvcache(at::Tensor &q,                 // batch_size x seqlen_q x num_heads x head_size
                ...
                const std::optional<const at::Tensor> &k_, // batch_size x seqlen_knew x num_heads_k x head_size
                const std::optional<const at::Tensor> &v_, // batch_size x seqlen_knew x num_heads_k x head_size
                const std::optional<const at::Tensor> &seqlens_k_, // batch_size
                const std::optional<const at::Tensor> &rotary_cos_, // seqlen_ro x (rotary_dim / 2)
                const std::optional<const at::Tensor> &rotary_sin_, // seqlen_ro x (rotary_dim / 2)
                const std::optional<const at::Tensor> &cache_batch_idx_, // indices to index into the KV cache
                const std::optional<const at::Tensor> &leftpad_k_, // batch_size
                const at::Tensor &block_table_,               // batch_size x max_num_blocks_per_seq
...
mha_fwd_kvcache_mla(
    ...
    return flash::mha_fwd_kvcache(q, kcache, kcache, \
                           ...
                           softmax_scale, is_causal, -1, -1, 0.0, false, 0);
