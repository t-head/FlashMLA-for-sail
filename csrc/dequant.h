...
struct KVCacheGmemBf16SimAIU {
    ...
    static constexpr int kBlockKSmem = 64;
    static constexpr int kSwizzle = 3;
    static constexpr int kHeadDim = kHeadDim_; // 576 or 512
    static constexpr int kHeadDimV = kHeadDimV_; // 512
    static constexpr int kGmemElemsPerLoad = sizeof(cute::uint128_t) / sizeof(ElementKVCache);
    static constexpr int kGmemThreadsPerRow = kBlockKSmem / kGmemElemsPerLoad;
...
struct KVCacheG2SFP8 {
    ...
    KVCacheG2SFP8(int tidx)
        : tidx(tidx) {
        tKgK_nope = gmem_tiled_copy_K_nope.get_thread_slice(tidx).partition_S(TensorKNope{}); //((_16,_1),_4,_1):
        tKgK_rope = gmem_tiled_copy_K_rope.get_thread_slice(tidx).partition_S(TensorKRope{}); //((_2,_1),_4,_1)
    };
