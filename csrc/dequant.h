...
__device__ __forceinline__ void fp8_to_bf16(Tensor0 src, Tensor1 dst, float scale) {
    ...
    Tensor dst_bf16x2 = recast<__nv_bfloat162>(dst);
    __nv_bfloat162 scale_bf16x2 = __float2bfloat162_rn(scale);
    __nv_bfloat162 dob = __float2bfloat162_rn(2.0);
    __nv_bfloat162 eps = __float2bfloat162_rn(-0.015625);
    #pragma unroll
    for (int m = 0; m < size(src_32b); ++m) {
        ...
        __nv_bfloat162 y0 = __hfma2(dst_bf16x2[m*2 + 0], dob, eps);
        __nv_bfloat162 y1 = __hfma2(dst_bf16x2[m*2 + 1], dob, eps);
...
struct KVCacheG2SFP8 {
    ...
    using Gmem_copy_struct_nope = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>;
    using Gmem_copy_struct_rope = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>;
    using GmemTiledCopyNope = decltype(
    ...
    using FragScale = decltype(make_fragment_like<float>(Layout<Shape<_4, _2>>{}));
    ...
    CUTLASS_DEVICE
    KVCacheG2SFP8(int tidx, int* gIndices_ptr, ElementKVCache* gK_ptr,
        const int block_size, const index_t batch_stride, const index_t row_stride)
        : tidx(tidx), block_size(block_size), batch_stride(batch_stride), row_stride(row_stride),
         gIndices_ptr(gIndices_ptr), gK_ptr(gK_ptr)
    {
        ...
        TensorKNope gK_nope = make_tensor(make_gmem_ptr(static_cast<ElementKVCache*>(nullptr)),
            Shape<Int<kBlockN>, Int<kHeadDimV>>{}, Stride<Int<BytesPerToken>, _1>{});
        tKgK_nope = gmem_tiled_copy_K_nope.get_thread_slice(tidx).partition_S(gK_nope); //((_16,_1),_4,_1):
        TensorKRope gK_rope = make_tensor(make_gmem_ptr(static_cast<Element*>(nullptr)),
            Shape<Int<kBlockN>, Int<kHeadDimRope>>{}, Stride<Int<BytesPerToken>, _1>{}); // (_64,_64)
        tKgK_rope = gmem_tiled_copy_K_rope.get_thread_slice(tidx).partition_S(gK_rope); //((_2,_1),_4,_1)
    };

    template <typename Tensor0, typename Tensor1>
    __forceinline__ __device__ void
    load_g2s_async(int n_block, int kv_store_num, Tensor0& smem_valid_indices, Tensor1& sK) {
        ////////////////////////////////////////////////////////////////////////////////
        // USE 8
        ////////////////////////////////////////////////////////////////////////////////
        const int thread_per_row = 8;
        const int load_col_idx = tidx / thread_per_row;
        #if ACOMPUTE_VERSION ==10000
        const int col_in_indices = (load_col_idx % 4) * 16 + (load_col_idx / 16) * 4 + (load_col_idx % 16) / 4;
        #else
        const int col_in_indices = load_col_idx;
        ...
        Tensor tKsK_rope = gmem_tiled_copy_K_rope.get_thread_slice(tidx).partition_D(sK_rope);
        constexpr int loop_num = kBlockN / (kNThreads / thread_per_row);
        for (int i = 0; i < loop_num; ++i) {
            int row_offset_indices = load_col_idx + n_block * kBlockN + i * (kNThreads / thread_per_row);
            int token_index = __ldg(gIndices_ptr + row_offset_indices);
            bool is_token_valid = token_index >= 0;
            smem_valid_indices(kv_store_num % 2, col_in_indices) = is_token_valid;
            if (!is_token_valid) {
            ...
            int block_index = token_index / block_size;
            int rel_idx_in_block = (token_index + block_size) % block_size;
            ElementKVCache* gK_nope = gK_ptr + block_index * batch_stride + rel_idx_in_block * row_stride;
            Element* gK_rope = (Element*)(gK_nope + kHeadDimV + 4 * sizeof(float));

            float* gK_scale = (float*)(gK_nope + kHeadDimV);
            load_128b_from_gmem<float, 4>(gK_scale, &tKrScale(0, kv_store_num % 2));

            tKgK_nope.data() = gK_nope + (tidx % thread_per_row) * 16;
            cute::copy(gmem_tiled_copy_K_nope, tKgK_nope(_, 0, _), tKsK_nope(_, i, _));

            tKgK_rope.data() = gK_rope + (tidx % thread_per_row) * 8;
            cute::copy(gmem_tiled_copy_K_rope, tKgK_rope(_, 0, _), tKsK_rope(_, i, _));
        }
    };
