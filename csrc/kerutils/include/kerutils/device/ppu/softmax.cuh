...
// File Path: /usr/local/cuda/include/math_constants.h
#define HGGCRT_INF_F __int_as_float(0x7f800000)
#define CUDART_L2E_F 1.4426950408889634f
...
// Group-level barrier sync using PTX bar.sync (synchronizes only a subset of threads)
// barrier_id: named barrier number (1-15, 0 is reserved for __syncthreads)
// num_threads: number of threads in the group to synchronize
__device__ __forceinline__ void group_barrier_sync(int barrier_id, int num_threads) {
...
__device__ __forceinline__ void reduce_between_2warps(Operator &op, Tensor<Engine0, Layout0> &row,
    ...
    const int barrier_id, const int num_threads) {
    ...
    // __syncthreads();
    group_barrier_sync(barrier_id, num_threads);
...
template<int kNRows, int WarpsK, bool UseTbSync = false, typename Engine0, typename Layout0, typename Engine1, typename Layout1>
__device__ __forceinline__ void reduce_sum_via_warps(Tensor<Engine0, Layout0> &row_sum,
    ...
    if constexpr (UseTbSync) {
        // loop-outside reduce_sum: must use threadblock sync (PPU named-barrier phase residual otherwise)
        __syncthreads();
    } else {
        group_barrier_sync(barrier_id, num_threads);
    }
...
__device__ __forceinline__ void reduce_max_between_warps(
    ...
    if constexpr(WarpsK == 2) {
        reduce_between_2warps<kNRows>(max_op, row_max, smem_row_max,
            row_idx_offset, warp_id_n, barrier_id, num_threads);
    } else {
...
template <bool USE_MMA_8, int kBlockM, int AtomLayoutQ, int AtomLayoutP, int WarpsK, bool ForceUseTsm = 0>
struct SoftmaxBetweenWarps {
    ...
    static constexpr bool StoreScalesTSM = ForceUseTsm || (AtomLayoutP != AtomLayoutQ);
    ...
    // Group barrier: each group of WarpsK warps uses a dedicated named barrier
    const int group_barrier_id = 1 + (warp_id % AtomLayoutQ);  // barriers 1-4 for 4 groups
    const int group_num_threads = WarpsK * 32;                  // threads per group
        ...
        if constexpr (Is_first) {
            ...
            if constexpr (WarpsK > 1) {
                flash::reduce_max_between_warps<kNRowsQ, WarpsK>(
                    ...
                    group_barrier_id, group_num_threads);
            }
            ...
            if constexpr (!StoreScalesTSM) {
                // use fa4 for AtomLayoutP == AtomLayoutQ.
                #pragma unroll
                for (int mi = 0; mi < kNRowsQ; ++mi) {
                    ...
                    float mid_scale = (scores_max_prev(mi) - scores_max_cur) * softmax_scale_log2;
                    if (mid_scale >= -8.0) {
                        row_max(mi) = (row_max(mi) == -INFINITY) ? 0.0f : scores_max_prev(mi);
                        scores_scale(mi) = 1.0f;
                    } else {
                        scores_scale(mi) = exp2f(mid_scale);
    ...
    __forceinline__ __device__ void softmax_rescale_o(Tensor0 &acc_o, Tensor1 &smem_row_scale) {
        ...
        if constexpr (!StoreScalesTSM) {
            bool thread_need_rescale = false;
            #pragma unroll
            for (int mi = 0; mi < size(scores_scale); ++mi) {
                if (scores_scale(mi) < 1.0f) {
                    thread_need_rescale = true;
                    break;
                }
            }
            bool warp_need_rescale = __any_sync(0xffffffff, thread_need_rescale);
            if (warp_need_rescale) {
                #pragma unroll
                for (int mi = 0; mi < kNRowsQ; ++mi) {
                    #pragma unroll
  ...
  template<bool Split, bool UseTbSync = true, typename Tensor0, typename Tensor1>
    __forceinline__ __device__ void normalize_softmax_lse_per_warp(
        Tensor0 &smem_row_sum, Tensor1 &smem_row_scale, float softmax_scale, const float* sink = nullptr, float sm_scale_div_log2 = 0, int m_block = 0) {

        SumOp<float> sum_op;
        quad_allreduce_(row_sum, row_sum, sum_op);
        if constexpr (WarpsK > 1) {
            flash::reduce_sum_via_warps<kNRowsQ, WarpsK, UseTbSync>(
        ...
        for (int mi = 0; mi < kNRowsQ; ++mi) {
            ...
            if (sink == nullptr) {
                ...
                scores_scale(mi) = (sum == 0.f || sum != sum) ? 1.f : sum;
                int q_row_idx = m_block * kBlockM + (warp_id % AtomLayoutQ) * MMA_ATOM_M + (mi % 2) * 8 + (threadIdx.x % 32 / 4) + (mi / 2) * AtomLayoutQ * MMA_ATOM_M;
                float attn_sink = sink == nullptr ? -HGGCRT_INF_F : sink[q_row_idx]*CUDART_L2E_F;
                float cur_max = row_max(mi) == -INFINITY ? 0.0f : row_max(mi);
                scores_scale(mi) = 1.f / (scores_scale(mi) + exp2f(attn_sink - cur_max * sm_scale_div_log2));
            }
            lse(mi) = (sum == 0.f || sum != sum) ? (Split ? -INFINITY : INFINITY) : row_max(mi) * softmax_scale + __logf(sum);
        }
