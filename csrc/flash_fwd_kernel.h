...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    /// make all lanes in a warp get the same smem_tiled_copy_K/V
    const int warp_idx = __ppu_read_firstlane(tidx / 32);
    const int tid_thread_slice = warp_idx * 32;

    // K
    auto tKgK_data = tKgK.data();
    Tensor gK = make_mix_tensor_like(make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.k_ptr) + row_offset_k),
                            Shape<Int<kBlockN>, Int<kHeadDim>>{},
                            make_stride(params.k_row_stride, _1{})));
    constexpr int kRowsPerAiuLoad = Kernel_traits::kRowsPerAiuLoad;
    typename Kernel_traits::GmemTiledCopyKV gmem_tiled_copy_K;
    ...
    if (block_table != nullptr) {
        /// copy kRowsPerAiuLoad rows each time
        gmem_tiled_copy_K.desc_ = AiuDesc{nullptr, kBlockN, params.k_row_stride, kRowsPerAiuLoad, Kernel_traits::kBlockKSmem, aiu_offset_k};
    } else {
    ...
    // V
    auto tVgV_data = tVgV.data();
    Tensor gV = make_mix_tensor_like(make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.v_ptr) + row_offset_v),
                            Shape<Int<kBlockN>, Int<kHeadDim>>{},
                            make_stride(params.v_row_stride, _1{})));

    typename Kernel_traits::GmemTiledCopyKV gmem_tiled_copy_V;
    ...
    int aiu_offset_v = Is_even_K ? 0 : (params.v_row_stride - params.d);
    if (block_table != nullptr) {
        /// copy kRowsPerAiuLoad rows each time
        gmem_tiled_copy_V.desc_ = AiuDesc{nullptr, kBlockN, params.v_row_stride, kRowsPerAiuLoad, Kernel_traits::kBlockKSmem, aiu_offset_v};
    } else {
        gmem_tiled_copy_V.desc_ = AiuDesc{nullptr, kBlockN, params.v_row_stride, kBlockN, Kernel_traits::kBlockKSmem, aiu_offset_v};
    }
    auto smem_tiled_copy_V = make_tiled_copy_B(typename Kernel_traits::SmemCopyAtomKVt{}, tiled_mma);
...
#if USE_AIU
    /// copy right most K
    /// should clear the sK smem for the tail, because there may nan in un-initialed tsm space
    FA_LOG_FMT("cp first K: Is_even_MN=%d, max_MN=%d, actual_seqlen_k=%d, n_block=%d, kBlockN=%d\n", Is_even_MN, binfo.actual_seqlen_k - n_block * kBlockN, binfo.actual_seqlen_k, n_block, kBlockN);
    FLASH_NAMESPACE::copy_kv_aiu<Is_even_MN, /* Clear_OOB_MN */true, Kernel_traits>(gmem_tiled_copy_K, tKgK, tKsK, tKVcKV, tKVpKV,
            kHeadDim, kRowsPerAiuLoad, gK, sK, n_block_max, block_table, params.page_block_size, params.k_batch_stride, params.k_row_stride, binfo.actual_seqlen_k - n_block * kBlockN);
#else
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            FA_LOG_FMT("masking: cp next V\n");
            FLASH_NAMESPACE::copy_kv_aiu<true, false, Kernel_traits>(gmem_tiled_copy_V, tVgV, tVsV, tKVcKV, tKVpKV,
                    kHeadDim, kRowsPerAiuLoad, gV, sV,
                    n_block + 1, block_table, params.page_block_size, params.v_batch_stride, params.v_row_stride);
        } else {
            // Clear the smem tiles to account for predicated off loads
            FA_LOG_FMT("masking: cp first V\n");
            FLASH_NAMESPACE::copy_kv_aiu<Is_even_MN, /*Clear_OOB_MN*/true, Kernel_traits>(gmem_tiled_copy_V, tVgV, tVsV, tKVcKV, tKVpKV,
                    kHeadDim, kRowsPerAiuLoad, gV, sV,
                    n_block + 1, block_table, params.page_block_size, params.v_batch_stride, params.v_row_stride, binfo.actual_seqlen_k - n_block * kBlockN);
        }
#else
        if (masking_step > 0) {
            if (block_table == nullptr) {
        ...
        cute::cp_async_fence();
#if USE_AIU
        FLASH_NAMESPACE::gemm_kv_aiu<false, false, Kernel_traits>(
        ...
        if (n_block > n_block_min) {
            ...
            FA_LOG_FMT("masking: cp next K\n");
            FLASH_NAMESPACE::copy_kv_aiu<true, false, Kernel_traits>(gmem_tiled_copy_K, tKgK, tKsK, tKVcKV, tKVpKV,
                    kHeadDim, kRowsPerAiuLoad, gK, sK, n_block, block_table, params.page_block_size,
...
#ifdef USE_PPU
        Tensor rP = FLASH_NAMESPACE::convert_acc<Element>(acc_s);
...
#endif
#if USE_AIU
        FLASH_NAMESPACE::gemm_rs_kv_aiu<Kernel_traits>(acc_o, tOrP, tOrVt, tOsVt, tiled_mma, smem_tiled_copy_V, smem_thr_copy_V);
#else
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        if (n_block > n_block_min) {
	        ...
	        if (block_table == nullptr) 
                tKgK.data() = tKgK.data() + (-int(kBlockN * params.k_row_stride));
            FA_LOG_FMT("cp next K\n");
            FLASH_NAMESPACE::copy_kv_aiu<true, false, Kernel_traits>(gmem_tiled_copy_K, tKgK, tKsK, tKVcKV, tKVpKV,
                    kHeadDim, kRowsPerAiuLoad, gK, sK, n_block, block_table, params.page_block_size, params.k_batch_stride, params.k_row_stride);
