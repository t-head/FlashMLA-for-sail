...
flash_fwd_splitkv_mla_combine_kernel(__grid_constant__ const Flash_fwd_params params) {
    ...
    if (warp_idx == 0) {
        ...
        if (params.attn_sink_ptr != nullptr) {
            int q_head_idx = hs_idx % params.ngroups;
            float attn_sink = __ldg(params.attn_sink_ptr + q_head_idx);
            if (global_lse != INFINITY) {
                global_lse += logf(1 + expf(attn_sink - global_lse));
            } else {
                global_lse = attn_sink == -INFINITY ? +INFINITY : attn_sink;
            }
        }
