...
def gen_testcase() -> List[RawTestParam]:
                ...
                for have_topk_len in ([False, True] if d_qk == 512 else [False]):
                    for h_q in [64, 128]:
                        cur_correctness_cases = [
                            RawTestParam(b, h_q, s_q, 1, s_k, is_varlen, topk, is_fp8,
    ...
    first_case = [RawTestParam(321, 64, 3, 1, 1024, True, 576,
                                have_topk_length=False,
                                enable_attn_sink=True,
                                extra_s_k=2046,
                                extra_topk=2048,
                                block_size=61,
                                extra_block_size=576,
...
def test_flash_mla(p: TestParam) -> Result:
    ...
    if p.num_runs == 0:
        ...
        splitkv_kernel_name = "flash_sparse_decode_fwd_kernel"
        combine_kernel_name = "flash_fwd_splitkv_mla_combine_kernel"
